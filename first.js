...
form.addEventListener("submit",(event)=>{
   ...
   const firstmsg=zodiacSigns[Month-1];
   const secondmsg=compliments[Day-1];
   const number1= ((SurName.length)*2);
   const thirdmsg=victimCardCompliments[number1]
    const number2=(31-Day)
   const fourthmsg=recommendations[number2]
   const number3=(Day*Month*Year)%20
   const fifthmsg = predictions[number3];

   const result=document.getElementById("result");
   result.innerHTML=`Hello ${Name} ${SurName}. your zodiac sign is ${firstmsg} ${secondmsg} ${thirdmsg} ${fourthmsg} ${fifthmsg} <br> Best of luck. we will concern you`;
